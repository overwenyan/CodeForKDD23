...
parser.add_argument('--log_fh', type=str, default='', help='file handle for logging files')
...
if __name__ == '__main__':
    ...
    save_name = f'search_algo_{args.dataset}_log'
    # args.log_fh = f'ablation_log/{save_name}.txt'
    args.log_fh = f'../results/ablation_log/{save_name}.txt'
    # store some hparams-perf pairs
    # we don't need subgraph here  maybe we can try some multi process technique here
    hostname = socket.gethostname()
    avaliable_device_ids = [6] # a100
    ...
    with open('results_meta.txt', 'a', encoding='utf-8') as f:
        ...
        f.writelines('recall20_list_meta = ' + str(recall20_list_meta) + '\n')
    ...
    with open('results_arch_meta.txt', 'a', encoding='utf-8') as f:
        f.writelines('recall20_list_origin = ' + str(recall20_list_origin) + '\n')
        f.writelines('recall20_list_meta = ' + str(recall20_list_meta) + '\n')
    ...
    plt.savefig(os.path.join('../figures/ablation_figs', f'meta_arch_ablation_{args.dataset}_{current_time}.jpg'))
    plt.close()
    ...
    plt.savefig(os.path.join('../figures/ablation_figs', f'meta_arch_learner_{meta_arch_learner.meta_mode}_{current_time}.jpg'))
    plt.close()
