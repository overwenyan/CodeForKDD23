...
parser.add_argument('--arch_num', type=int, default=138, help='arch seed')
parser.add_argument('--log_fh', type=str, default='', help='logging file handle')
parser.add_argument('--file_id', type=int, default=139, help='size of anchor sample number')
...
if __name__ == '__main__':
    ...
    arch_encode = remaining_arches_encoding[arch_num-1]
    
    arch_single = {'cf': 'ui', 'emb':{'u': 'lightgcn', 'i': 'lightgcn'}, 'ifc': 'mul', 'pred': 'i'}
    # arch_single['cf'], arch_single['emb']['u'], arch_single['emb']['i'], arch_single['ifc'], arch_single['pred'] = arch_encode.split('_')
    arch_single_list.append(arch_single)

    save_name = f'arch_{arch_num}_{arch_encode}_{args.dataset}_log'
    fh = logging.FileHandler(os.path.join('arch_log', save_name + '.txt'))
    fh.setFormatter(logging.Formatter(log_format))
    logging.getLogger().addHandler(fh)
    args.log_fh = f'arch_log/{save_name}.txt'
    logging.info('model type: {}'.format(arch_encode))
    stage1_start = time()
    ...
    for tasks in task_index:
        with mp.Pool(processes=len(tasks)) as p:
            ...
            p.name = 'test on arch tuning' # recall20: 0.1367, ndcg: 11.6330[10.0257]
            if args.data_type == 'implicit':
    ...
    meta_hp_classifier = MetaHPLearner(meta_mode='bore_rf') # bore_mlp
    ...
    args.device = avaliable_device_ids[0]
    ...
    plt.savefig(os.path.join('ablation_figs', f'meta_hp_ablation_{args.file_id}_{args.dataset}_{current_time}.jpg'))
